    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            access = Access(
                ...
                assessment_access=request.data.get("assessment_access"),
                attendance_access=request.data.get("attendance_access"),
                classes_access=request.data.get("classes_access"),
                fees_access=request.data.get("fees_access"),
                parents_access=request.data.get("parents_access"),
                payments_access=request.data.get("payments_access"),
                portal_access=request.data.get("portal_access"),
                reports_access=request.data.get("reports_access"),
                settings_access=request.data.get("settings_access"),
                staff_access=request.data.get("staff_access"),
                students_access=request.data.get("students_access"),
                subjects_access=request.data.get("subjects_access"),
                teachers_access=request.data.get("teachers_access"),
                terms_access=request.data.get("terms_access"),
            )
...
class AllActivityListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        school = self.request.query_params.get('user', None)
        if school is not None:
            queryset = queryset.filter(school__school=school)
        return queryset
